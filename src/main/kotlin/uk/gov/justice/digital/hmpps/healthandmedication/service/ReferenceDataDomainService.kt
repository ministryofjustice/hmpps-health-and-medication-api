...
class ReferenceDataDomainService(
  ...
  ): Collection<ReferenceDataDomainDto> = referenceDataDomainRepository.findAllByIncludeInactive(includeInactive, includeSubDomains)
    .map { it.toDto() }

  fun getReferenceDataDomain(code: String): ReferenceDataDomainDto = referenceDataDomainRepository.findByCode(code)
    ?.toDto()
