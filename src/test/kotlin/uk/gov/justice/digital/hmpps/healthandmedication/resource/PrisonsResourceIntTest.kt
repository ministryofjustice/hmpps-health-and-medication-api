  ...
  inner class GetHealthAndMedicationFilters {
    ...
    inner class GetDynamicFilters {
      ...
      inner class HappyPath {
        @Test
        @RepopulateDb
        fun `can get filters dynamically with no filters applied`() {
          val response = getDynamicFilters("")
        ...
        @Test
        @RepopulateDb
        fun `can get filters dynamically with a filter applied`() {
          val response = getDynamicFilters("?personalisedDietaryRequirements=KOSHER")
        ...
        @Test
        @RepopulateDb
        fun `can get filters dynamically with multiple filters applied`() {
          val response =
            getDynamicFilters("?foodAllergies=SOYA&medicalDietaryRequirements=LOW_CHOLESTEROL&topLocationLevel=A&recentArrival=true")
        ...
        private fun getDynamicFilters(queryParams: String): WebTestClient.ResponseSpec = webTestClient.get()
          .uri("/prisons/${PRISON_ID}/filters$queryParams")
          .headers(setAuthorisation(USER1, roles = listOf("ROLE_HEALTH_AND_MEDICATION_API__HEALTH_AND_MEDICATION_DATA__RO")))
          .header("Content-Type", "application/json")
          .exchange()
          .expectStatus().isOk
